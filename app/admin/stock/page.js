...
export default function StockDashboard() {
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-8">
        <h1 className="text-3xl font-bold tracking-tight text-zinc-900">Control de Lotes</h1>
        <p className="text-zinc-500 mt-1">Registra la entrada física de cajas al almacén.</p>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm p-6 mb-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="md:col-span-1">
            <label className="block text-sm font-medium text-zinc-700 mb-1">Medicamento</label>
          ...
          <div className="md:col-span-1">
            <label className="block text-sm font-medium text-zinc-700 mb-1">Folio del Lote</label>
            <input required type="text" name="batch_number" value={formData.batch_number} onChange={handleChange} placeholder="Ej. L-4091"
                   className="w-full bg-zinc-50 border border-zinc-200 text-zinc-900 text-sm rounded-lg focus:ring-zinc-900 focus:border-zinc-900 block p-2.5 outline-none transition-all" />
          </div>
          <div className="md:col-span-1">
            <label className="block text-sm font-medium text-zinc-700 mb-1">Cajas Entrantes</label>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm overflow-hidden">
        <table className="w-full text-sm text-left text-zinc-500">
          ...
          <tbody>
            {stocks.length === 0 ? (
              <tr><td colSpan="4" className="px-6 py-8 text-center text-zinc-400">Sin lotes en almacén</td></tr>
            ) : (
              stocks.map((stock) => (
                <tr key={stock.id} className="bg-white border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-zinc-900">{getMedicineName(stock.medicine_id)}</td>
                  <td className="px-6 py-4 text-zinc-600">{stock.batch_number}</td>
                  <td className="px-6 py-4 font-semibold text-emerald-600">{stock.quantity} cajas</td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleEditClick(stock)} className="font-semibold text-zinc-600 hover:text-zinc-900 underline mr-4">Editar</button>
